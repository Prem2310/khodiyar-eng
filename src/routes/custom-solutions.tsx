  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Custom Valve Engineering Solutions | Khodiyar" },
      {
        ...
        content: "Custom valve design, non-standard face-to-face dimensions, special alloys and project batch production.",
      },
    ],
  }),
    ...
    <>
      ...
      >
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          <Button asChild size="lg" variant="outline" className="border-white/30 text-navy-foreground hover:bg-white/10">
            <a
              href={whatsappLink("Hello Khodiyar Engineering, I want to discuss a custom valve requirement.")}
              target="_blank"
      ...
      <section className="py-16 sm:py-20">
        <div className="container-x grid gap-6 sm:grid-cols-2">
          {FEATURES.map((f) => (
            <div key={f.title} className="surface-panel rounded-sm p-7 flex flex-col justify-between bg-card hover:border-accent transition-colors">
              <div>
                <f.icon className="h-7 w-7 text-accent" />
                <h2 className="mt-4 font-display text-lg font-semibold uppercase tracking-wide">
                  {f.title}
                </h2>
                <p className="mt-2 text-xs sm:text-sm leading-relaxed text-muted-foreground">{f.text}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="bg-navy-gradient py-16 sm:py-20 text-navy-foreground">
        <div className="container-x">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { step: "1. Drawing Review", desc: "We review your line schematic, fluid media, operating envelope, and dimensional constraints." },
              { step: "2. CAD & GA Drawing", desc: "Our design desk prepares a General Arrangement (GA) drawing with cross-section for approval." },
              { step: "3. Precision CNC Run", desc: "Machined from certified billets/castings on CNC centres to tight tolerances." },
              { step: "4. Certified Testing", desc: "100% shell hydro tested, seat leak verified, and shipped with complete EN 10204 3.1 documentation." },
            ].map((p) => (
              <div key={p.step} className="rounded-sm border border-white/15 bg-white/5 p-5 backdrop-blur-sm">
